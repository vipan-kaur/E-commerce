...
const Products = () => {
    ...
    `${item.description} ${item.title}`.toLowerCase().includes(search.toLowerCase())
  );
 ...
 return (
  ...
  <div className="grid  bg-black grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
    {filter.map((item) => (
      <div
        key={item.id}
      ...
      >
       ...
       <Link to={`/buy/${item.id}`}> <button className='border rounded-2xl p-3 mt-3  '>Buy now</button></Link>
